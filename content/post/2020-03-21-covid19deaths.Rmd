...
The other countries are a bit bunched, so next I'll limit the plot to countries with fewer than 5 deaths per million yesterday.


## Countries with fewer than 5 deaths per million yesterday

```{r lt5-deaths-per-mil}
ggplot(
  filter(data, deaths_per_million_smoothed_yesterday < 5), 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
    colour = deaths_per_million_smoothed_yesterday,
...
* **Belgium** and **The Netherlands** are growing at alarming rates and are currently heading for 3 deaths per million per day. They would be doing well not to breach 10 deaths per million per day soon. 
* **France**'s data is weirdly oscillating but they are currently showing a stark increase in deaths per million per day.
* Interestingly, **Iran** look to be turning the corner and lowering their fatalities. We'll need another week of data to see if they can sustain this. 
...
```{r lt1-death-per-mil}
ggplot(
  filter(data, deaths_per_million_smoothed_yesterday < 1), 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
    colour = deaths_per_million_smoothed_yesterday,
...
* For **Ecuador**, **Portugal**, **Sweden** and **Greece**, it's too early to tell, though the upward trajectories are cause for great concern. 
* **Germany** appeared to be tackling the disease well earlier in the week, but their deaths are increasing again now.
* The **USA** curve looks pretty ominous here. They have 3 main states (California, New York and Washington) driving most of their cases and deaths. See below for a breakdown of the US by states.
...
I'm going to use the same limitations for the US state data except that I'll allow populations over 7.5 million, so that we can see Washington state.
...
ggplot(
  ...
  labs(x = "days since tenth death", y = "deaths per million",
       colour = "deaths/million\nyesterday") +
  scale_colour_gradient(low = "blue", high = "red") + 
  xlim(0, max(us_data$days_since_tenth_death) + 2) + 
  theme(text = element_text(size = 20))
```
...
* For **Ohio**, **Virginia**, **Pennsylvania** and **Michigan**, it's too early to say.
...
* **New York** is at `r round(ny_current_deaths_per_million)` deaths per million `r ny_days_since_tenth_death` days after its tenth death. At this point, Spain was at `r round(spain_ny_equivalent_deaths_per_million)` deaths per million. New York looks like it will get much worse than Washington.
* **New Jersey** is at `r round(nj_current_deaths_per_million)` deaths per million `r nj_days_since_tenth_death` days after its tenth death; better than New York but not good at all. At this point, Spain was at `r round(spain_nj_equivalent_deaths_per_million)` deaths per million. 
* **Georgia** and **Illinois** are near where New York was at equivalent stages, but with flatter trajectories. But New York is a low bar. To be barely better than New York is to be headed for disaster.
* `r round(ca_days_since_tenth_death)` days after its tenth death, **California** is at approximately `r round(ny_current_deaths_per_million)` `r if_else(round(ny_current_deaths_per_million) == 1, "death", "deaths")` per million. This is similar to where Italy was at this stage, but California's curve is flatter than Italy's, so they are on course to avoid the explosion in cases that happened there.
* **Texas** and **Florida** look very like California with a few days' lag.
