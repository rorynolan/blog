...
## States with fewer deaths per million than Italy yesterday

```{r us-plot-less-than-italy}
italy_deaths_per_million_smoothed_yesterday <- us_plot_data %>% 
  dplyr::filter(state == "ITA") %>% 
  pull(deaths_per_million_smoothed_yesterday) %>% 
  unique()
us_plot_data_less_than_italy <- us_plot_data %>% 
  dplyr::filter(
    deaths_per_million_smoothed_yesterday <
      italy_deaths_per_million_smoothed_yesterday
  )
ggplot(
  us_plot_data_less_than_italy, 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
    colour = deaths_per_million_smoothed_yesterday,
  ...
  labs(x = "days since tenth death", y = "deaths per million",
       colour = "deaths\nper\nmillion\nyesterday") +
  scale_colour_gradient(low = "blue", high = "red") + 
  xlim(0, max(us_plot_data_less_than_italy$days_since_tenth_death) + 2) + 
  theme(text = element_text(size = 20))
```
