...
Due to the long (approximately 2-week) latency period of the virus (_latency_ is roughly the period between a person encountering the virus and showing symptoms of infection), the benefit of social distancing in these statistics is seen at least two weeks after its inception. 
...
```{r italy-yesterday}
italy_deaths_per_million_smoothed_yesterday <- data %>% 
  dplyr::filter(iso3c == "ITA") %>% 
  dplyr::filter(days_since_tenth_death == max(days_since_tenth_death)) %>% 
  pull(deaths_per_million_smoothed_yesterday)
```
...
## Countries at least as many deaths per million than Italy yesterday

```{r lt10-deaths-per-mil}
ggplot(
  dplyr::filter(
    data, 
    ceiling(deaths_per_million_smoothed_yesterday) >=
      floor(italy_deaths_per_million_smoothed_yesterday) 
  ), 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
...
## Countries with fewer deaths per million than Italy yesterday

```{r lt-ita-yesterday}
ggplot(
  dplyr::filter(
    data, 
    floor(deaths_per_million_smoothed_yesterday) <=
      ceiling(italy_deaths_per_million_smoothed_yesterday)
  ), 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
...
## Countries with fewer deaths per million than the USA yesterday

```{r usa-yesterday}
usa_deaths_per_million_smoothed_yesterday <- data %>% 
...
ggplot(
  dplyr::filter(
    data, 
    floor(deaths_per_million_smoothed_yesterday) <=
      ceiling(usa_deaths_per_million_smoothed_yesterday)
  ), 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
...
## States more more deaths per million than Washington yesterday

```{r us-plot-more-than-wash}
wash_deaths_per_million_smoothed_yesterday <- us_plot_data %>% 
  dplyr::filter(state == "WA") %>% 
  pull(deaths_per_million_smoothed_yesterday) %>% 
  unique()
us_plot_data_more_than_wash <- us_plot_data %>% 
  dplyr::filter(
    ceiling(deaths_per_million_smoothed_yesterday) >=
      floor(wash_deaths_per_million_smoothed_yesterday)
  )
ggplot(
  us_plot_data_more_than_wash, 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
    colour = deaths_per_million_smoothed_yesterday,
  ...
  labs(x = "days since tenth death", y = "deaths per million",
       colour = "deaths\nper\nmillion\nyesterday") +
  scale_colour_gradient(low = "blue", high = "red") + 
  xlim(0, max(us_plot_data_more_than_wash$days_since_tenth_death) + 2) + 
  theme(text = element_text(size = 20))
```


## States with fewer deaths per million than Washington yesterday

```{r us-plot-less-than-wash}
us_plot_data_less_than_wash <- us_plot_data %>% 
  dplyr::filter(
    floor(deaths_per_million_smoothed_yesterday) <=
      ceiling(wash_deaths_per_million_smoothed_yesterday)
  )
ggplot(
  us_plot_data_less_than_wash, 
  aes(
    x = days_since_tenth_death,
    y = deaths_per_million_smoothed,
    colour = deaths_per_million_smoothed_yesterday,
  ...
  labs(x = "days since tenth death", y = "deaths per million",
       colour = "deaths\nper\nmillion\nyesterday") +
  scale_colour_gradient(low = "blue", high = "red") + 
  xlim(0, max(us_plot_data_less_than_wash$days_since_tenth_death) + 2) + 
  theme(text = element_text(size = 20))
```
